...
BinaryTreeIterator::BinaryTreeIterator(TreeNode *root, BinaryTreeIterator::TraverseOrder order)
  : root_(root),
    order_(order)
{
  if (root) {
...
void BinaryTreeIterator::InOrderLeftmost(TreeNode *root) {
  while(root) {
    nodes_.push(root);
    root = root->left;
  }
...
void BinaryTreeIterator::PostOrder(TreeNode *root) {
  if (root) {
    nodes_.push(root);
